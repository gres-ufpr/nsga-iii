...
public class Niching<S extends Solution> {
    ...
    public Niching(List<S> population, List<S> lastFront, double[][] lambda, int remain, boolean normalization) {
    ...
    void normalizePopulation() {
        for (int i = 0; i < mgPopulation.size(); i++) {
            ...
            for (int j = 0; j < obj; j++) {
                ...
                double val = (sol.getObjective(j) - zideal[j]) / (intercepts[j] - zideal[j]);
                normalized_[j] = val;
            }
    ...
    public double calNormlizedVDistance(Solution sol, double[] ref) {
        ...
        for (int i = 0; i < sol.getNumberOfObjectives(); i++) {
            d2 += (normalized_[i] - d1 * (ref[i] / refLenSQ)) * (normalized_[i] - d1 * (ref[i] / refLenSQ));
        }
        d2 = Math.sqrt(d2);
    ...
    double calUnNormalizedVDistance(Solution sol, double[] ref) {
        ...
        for (int i = 0; i < sol.getNumberOfObjectives(); i++) {
            ...
            d2 += ((sol.getObjective(i) - zideal[i]) - d1 * (ref[i] / nl))
                    * ((sol.getObjective(i) - zideal[i]) - d1 * (ref[i] / nl));
        }
        d2 = Math.sqrt(d2);
